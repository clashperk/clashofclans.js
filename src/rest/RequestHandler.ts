...
export class RequestHandler {
    ...
    private readonly throttler?: BatchThrottler | QueueThrottler | null;

    private readonly cached: Store<{ data: unknown; status: number; ttl: number }> | null;
    ...
    private async exec<T>(path: string, options: RequestOptions = {}, _retries = 0): Promise<Response<T>> {
        let retries = 0;
        ...
        if (
            ...
            const keys = await this.login();
            if (keys.length) return this.exec<T>(path, options, ++retries);
        }

        const maxAge = Number(res?.headers.get('cache-control')?.split('=')?.[1] ?? 0) * 1_000;
    ...
    public async init(options: LoginOptions) {
        if (!(options.email && options.password)) throw new ReferenceError('Missing email and password.');
    ...
    private async getKeys(cookie: string, ip: string) {
        ...
        const keys = (data.keys ?? []) as { cidrRanges?: string[]; id: string; key: string; name: string }[];
    ...
    private async createKey(cookie: string, ip: string) {
        ...
        return data.key as { cidrRanges?: string[]; id: string; key: string; name: string };
    }
