...
export class WarClan {
    ...
    public getMember(tag: string): ClanWarMember | null {
        return this.members.find((member) => member.tag === tag) ?? null;
    }
...
export class ClanWar {
     ...
     */
    public state: 'inWar' | 'preparation' | 'warEnded';
    ...
    public constructor(client: Client, data: APIClanWar, extra: { clanTag?: string; maxAge: number; warTag?: string }) {
